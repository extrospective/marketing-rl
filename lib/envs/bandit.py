...
class BanditEnv(Environment):
    def __init__(self, num_actions = 10, distribution = "bernoulli", evaluation_seed="387"):
        ...
        # MVA
        print('Reward parameters are randomized to :')
        print(self.reward_parameters)
        print('Average reward:')
        print(np.mean(self.reward_parameters))
        print('Maximum reward:')
        print(np.max(self.reward_parameters))
        print('Difference between maximum and average is:')
        print(np.round(np.max(self.reward_parameters) - np.mean(self.reward_parameters), 3))
