...
class Experiment(object):
    def __init__(self, env, agent, show_episode_length=True, show_episode_reward=True):
        ...
        self.ax = pylab.subplot(gs[0, 0]) # :,0
        ...
        self.ax1 = pylab.subplot(gs[1, 0])  # 0,1
        ...
        self.ax2 = pylab.subplot(gs[2, 0])  # 1,1
    ...
    def run_bandit(self, max_number_of_trials=1000, display_frequency=1):
        ...
        # MVA
        print("Maximum arm pulls", np.max(self.agent.total_counts))  
        print("Position of maximum arm pulls", np.argmax(self.agent.total_counts))  
